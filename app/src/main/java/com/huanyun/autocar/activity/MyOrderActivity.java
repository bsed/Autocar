...
import com.huanyun.autocar.adapter.MyOrderAdapter;
...
public class MyOrderActivity extends BaseActivity implements SwipeRefreshLayout.OnRefreshListener,
	...
	public static final String TAG = CarWashActivity.class
			.getSimpleName();
	public static MyOrderActivity carRepairActivity;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		latitude = CommonSettingProvider.getLatitude(MyOrderActivity.this);
		longitude = CommonSettingProvider.getLongitude(MyOrderActivity.this);
	...
	private void initView() {
		...
		findView(R.id.no_data_tv).setVisibility(View.GONE);
		initData();
		onRefresh();
	...
	private void initData(){
		...
		if(myOrderAdapter==null){
			myOrderAdapter = new MyOrderAdapter(MyOrderActivity.this,workData);
		}
	...
	private void getOrderList(int pageNo){
		Api api = new Api(MyOrderActivity.this, new RequestCallBack<Object>() {
			@Override
			public void onStart() {
				super.onStart();
			}
			...
			public void onSuccess(ResponseInfo<Object> arg0) {
				...
				if (error == null || error.equals("")) {
					...
					if (page > 1) {
						myOrderAdapter.refreshData(workData);
						return;
					}

					listView.setAdapter(myOrderAdapter);
					myOrderAdapter.refreshData(workData);
					myOrderAdapter.notifyDataSetInvalidated();

					if (workData.size() == 0) {
						// nothingImg.setVisibility(View.VISIBLE);
						findView(R.id.no_data_tv).setVisibility(View.VISIBLE);
					} else {
		...
		api.getMyOrder(CommonSettingProvider.getUserId(MyOrderActivity.this),pageNo);
	}
